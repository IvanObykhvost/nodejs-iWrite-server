        ...
        this.addOrDeleteFollow = (req, res, action) => {
            ...
            this._userRepository.findUsers({ $or: [{ token }, { name }] })
                ...
                if (action === constants_1.constants.operation.add_follower) {
                    ...
                    sortUsers[0].followings = sortUsers[0].followings.filter(el => el.toString() !== sortUsers[1].id.toString());
                    sortUsers[1].followers = sortUsers[1].followers.filter(el => el.toString() !== sortUsers[0].id.toString());
                }
